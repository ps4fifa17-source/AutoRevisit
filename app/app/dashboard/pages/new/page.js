...
export default function NewCustomerPage() {
  ...
  const [includeDealerContext, setIncludeDealerContext] = useState(false);
  ...
  const allowedPushIds =
    plan.id === "premium"
      ? Object.keys(PUSH_OPTIONS)
      : plan.id === "pro"
      ? ["reassurance", "value", "first_car", "finance", "family", "performance"]
      : ["reassurance", "value", "first_car"];
  const allowedStyleIds =
    plan.id === "premium"
      ? Object.keys(DESIGN_STYLES)
      : plan.id === "pro"
      ? ["clean_light", "safe_light", "finance_focus", "warm_family", "dark_premium"]
      : ["clean_light", "safe_light"];
  const allowedPushOptions = Object.fromEntries(allowedPushIds.map((id) => [id, PUSH_OPTIONS[id]]).filter(([, option]) => option));
  const allowedDesignStyles = Object.fromEntries(allowedStyleIds.map((id) => [id, DESIGN_STYLES[id]]).filter(([, option]) => option));
  ...
  function updatePush(value) {
    const nextRecommended = getRecommendedStyle(value);
    const nextStyle = allowedStyleIds.includes(nextRecommended) ? nextRecommended : "clean_light";
    setForm((current) => ({
      ...current,
      push_angle: value,
      design_style: nextStyle,
    }));
  }
  ...
  async function createPage() {
    ...
    const financePayload = includeFinance
      ? {
    ...
    const dealerContext = includeDealerContext ? form.dealer_notes : "";
      ...
      .insert({
        ...
        notes:
          form.page_type === "enquiry"
            ? `Enquiry follow up${dealerContext ? ` | Notes: ${dealerContext}` : ""}`
            : `Who for: ${form.who_for} | Focus: ${form.push_angle} | Style: ${form.design_style}${dealerContext ? ` | Notes: ${dealerContext}` : ""}`,
      })
      .select()
      .single();
          ...
          <div className="grid xl:grid-cols-[1fr_430px] gap-5">
            <section className="space-y-5">
              ...
              {form.page_type === "revisit" && (
                <>
                  <Panel icon={UserRound} badge="Guidance" title="3. Guide the AI">
                    <p className="text-ink/55 mt-3">Defaults are already set. Change only what helps this customer.</p>
                    {plan.id !== "premium" && <p className="text-sm text-ink/45 mt-2">Some focus types are unlocked on higher packages.</p>}
                    <div className="grid md:grid-cols-2 gap-4 mt-6">
                      <SelectField
                        label="Buying for"
                        value={form.who_for}
                        onChange={(value) => update("who_for", value)}
                        options={WHO_FOR_OPTIONS}
                      />
                      <SelectField
                        label="Page focus"
                        value={form.push_angle}
                        onChange={updatePush}
                        options={allowedPushOptions}
                      />
                    </div>
                  </Panel>
                  ...
                  <FinancePanel
                    ...
                    enabled={includeFinance}
                    onToggle={setIncludeFinance}
                  />

                  <NotesPanel
                    form={form}
                    update={update}
                    title={`5. ${DEALER_NOTES_TITLE}?`}
                    placeholder={DEALER_NOTES_PLACEHOLDER_REVISIT}
                    enabled={includeDealerContext}
                    onToggle={setIncludeDealerContext}
                  />

                  <Panel icon={Palette} badge="Style" title="6. Page style">
                    <p className="text-ink/55 mt-3">
                      Recommended: <span className="font-black text-ink">{DESIGN_STYLES[recommendedStyle]?.label}</span>. Most dealers can leave this as-is.
                    </p>
                    <div className="rounded-[26px] bg-white/70 border border-ink/10 p-4 mt-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                      <div>
                        <p className="text-xs uppercase tracking-[0.14em] font-black text-ink/35">Selected style</p>
                        <p className="font-black text-2xl mt-1">{DESIGN_STYLES[form.design_style]?.label || DESIGN_STYLES.clean_light.label}</p>
                        <p className="text-sm text-ink/50 mt-1">{DESIGN_STYLES[form.design_style]?.description || DESIGN_STYLES.clean_light.description}</p>
                      </div>
                      <button type="button" onClick={() => setShowStylePicker((value) => !value)} className="btn-secondary shrink-0">
              ...
              {form.page_type === "enquiry" && (
                <NotesPanel
                  form={form}
                  update={update}
                  title={`3. Add extra enquiry context?`}
...
function FinancePanel({ form, update, title, enabled, onToggle }) {
  return (
    <Panel icon={WalletCards} badge="Finance" title={title}>
      <ToggleButtons
        ...
        onChange={onToggle}
        yes="Yes, add finance"
        no="No finance details"
      />
      {enabled && (
        <>
          <p className="text-ink/55 mt-4">Only add figures you actually have. Blank fields will not be guessed.</p>
...
function NotesPanel({ form, update, title, placeholder, enabled, onToggle }) {
  return (
    <Panel icon={StickyNote} badge="Your words" title={title}>
      ...
      {enabled && (
        <>
          <p className="text-ink/55 mt-4">Tell AutoRevisit what mattered in the conversation. This shapes the page around the customer and vehicle.</p>
          <textarea
            className="input min-h-[140px] mt-5"
...
function SelectField({ label, value, onChange, options }) {
  return (
    <label className="block">
      <span className="text-xs uppercase tracking-[0.14em] font-black text-ink/35">{label}</span>
      <select className="input mt-2" value={value} onChange={(e) => onChange(e.target.value)}>
        {Object.values(options).map((option) => (
          <option key={option.id} value={option.id}>{option.label}</option>
        ))}
      </select>
      <span className="block text-sm text-ink/50 mt-2">{options[value]?.description}</span>
    </label>
  );
}

function ToggleButtons({ value, onChange, yes, no }) {
  return (
    <div className="grid grid-cols-2 gap-3 mt-6">
      <button
        type="button"
        onClick={() => onChange(false)}
        className={`rounded-2xl px-4 py-4 font-black border transition ${!value ? "bg-ink text-acid border-ink" : "bg-white/70 border-ink/10 text-ink"}`}
      >
        {no}
      </button>
      <button
