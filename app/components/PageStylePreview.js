"use client";

import { DESIGN_STYLES } from "@/lib/pageJourney";
import { CheckCircle, Sparkles } from "lucide-react";

export default function PageStylePreview({ value, onChange, recommendedStyle, dealerName = "Dealer", vehicleTitle = "Vehicle", styles = DESIGN_STYLES }) {
  return (
    <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-3">
      {Object.values(styles).map((style) => {
        const active = value === style.id;
        const recommended = recommendedStyle === style.id;

        return (
          <button
            key={style.id}
            type="button"
            onClick={() => onChange(style.id)}
            className={`text-left rounded-[24px] border bg-white overflow-hidden transition ${
              active ? "border-ink ring-4 ring-acid/40" : "border-ink/10 hover:border-ink/25"
            }`}
          >
            <MiniPreview styleId={style.id} dealerName={dealerName} vehicleTitle={vehicleTitle} />
            <div className="p-4">
              <div className="flex items-center justify-between gap-2">
                <p className="font-black text-lg">{style.label}</p>
                {active ? <CheckCircle size={18} /> : recommended ? <Sparkles size={18} /> : null}
              </div>
              <p className="text-sm text-ink/50 mt-1">{style.description}</p>
              {recommended && <p className="inline-flex mt-3 rounded-full bg-acid px-3 py-1 text-xs font-black">Recommended</p>}
            </div>
          </button>
        );
      })}
    </div>
  );
}

function MiniPreview({ styleId, dealerName, vehicleTitle }) {
  if (styleId === "finance_focus") {
    return (
      <div className="h-36 bg-white text-ink p-4 flex flex-col justify-between">
        <p className="text-xs font-black uppercase tracking-[0.14em] text-black/45">Monthly view</p>
        <div>
          <h3 className="text-5xl font-black tracking-[-0.07em]">£219</h3>
          <p className="font-black text-sm">per month</p>
        </div>
      </div>
    );
  }

  if (styleId === "warm_family") {
    return (
      <div className="h-36 bg-[#fff7e8] text-ink p-4">
        <p className="font-black">Hello Jack,</p>
        <h3 className="text-2xl font-black mt-3 leading-tight">Built for daily family life.</h3>
        <div className="flex gap-2 mt-3 text-xs font-black"><span className="bg-white rounded-full px-3 py-1">Comfort</span><span className="bg-white rounded-full px-3 py-1">Space</span></div>
      </div>
    );
  }

  if (styleId === "safe_light") {
    return (
      <div className="h-36 bg-[#f8f9ff] text-ink p-4">
        <p className="text-xs font-black uppercase tracking-[0.14em] text-black/45">Confidence</p>
        <h3 className="text-3xl font-black mt-4 leading-tight">Why this could suit you.</h3>
      </div>
    );
  }

  if (styleId === "dark_premium") {
    return (
      <div className="h-36 bg-[#07080b] text-white p-4">
        <p className="text-xs font-black text-white/45">{dealerName}</p>
        <h3 className="text-3xl font-black italic leading-none mt-5">THIS<br />ONE<br />STANDS OUT</h3>
      </div>
    );
  }

  if (styleId === "executive_minimal") {
    return (
      <div className="h-36 bg-[#f4f4f1] text-ink p-4">
        <p className="text-xs tracking-[0.18em] uppercase font-black">Clear overview</p>
        <h3 className="text-3xl font-black mt-6 leading-none">{vehicleTitle}</h3>
        <div className="h-1 w-12 bg-black mt-3" />
      </div>
    );
  }

  if (styleId === "luxury_dark") {
    return (
      <div className="h-36 bg-[#070707] text-[#f5eddf] p-4">
        <p className="text-xs tracking-[0.22em] text-[#f5eddf]/45">PREPARED FOR YOU</p>
        <h3 className="text-3xl font-black mt-8 leading-none">{vehicleTitle}</h3>
      </div>
    );
  }

  return (
    <div className="h-36 bg-[#f7f7f4] text-ink p-4">
      <p className="font-black">{dealerName}</p>
      <h3 className="text-3xl font-black mt-7 leading-tight">Hello Jack, your page is ready.</h3>
    </div>
  );
}
